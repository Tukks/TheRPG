...
class ChoixStrategie {
	...
	/**
	 * Mise en place de l'encart qui va contenir le choix de stratégie
	 * 
	 * @param shell
	 * @param items
	 */
	ChoixStrategie(Shell shell, ChargementDynamique items) {
		...
		thisGroup.setText("Définir stratégie");
	...
	private void isPotionOrAttaque() {
		if (items.getTypeItem() == "Potion"
			...
			text.setText("Définir quand utiliser la Potion, en pourcentage par rapport au point de vie du Personnage : ");
			text.setSize(850, 15);
			...
			Text text = new Text(thisGroup, SWT.NONE);
			text.setText("Définir quand utiliser le Poison, en pourcentage par rapport au point de vie du Personnage : ");
			text.setSize(850, 15);
			text.setLocation(15, 15);
			text.setBackgroundImage(thisGroup.getBackgroundImage());
			choix = new Combo(thisGroup, SWT.DROP_DOWN | SWT.READ_ONLY);
